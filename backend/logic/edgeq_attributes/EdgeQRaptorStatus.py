...
class EdgeQRaptorStatus(Attribute):
    du_Check = "CELL_IS_UP, CELL_ID:1"


    def __init__(self, raptor_log_path):
        super().__init__()
        self.raptorStatus = RaptorStatusType.OFF
        self.raptor_log_path = raptor_log_path
    ...
    def check_Du_Log(self) -> bool:
        if not os.path.isfile(self.raptor_log_path):
            print("Log path {} does not exist.".format(self.raptor_log_path))
            return False

        # Call `tail -n 1` so you don't try to run the .txt itself
        cmd = ["tail", "-n", "1", self.raptor_log_path]
        try:
    ...
    def print_Raptor_Status(self):
        print(f"EdgeQ Raptor Status: {self.raptorStatus.name}")
